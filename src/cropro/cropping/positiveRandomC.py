...
class positiveRandomC:
    ...
    def positiveRandom(self):
        ...
        if acceptance_boolean:
            ...
            if (
                not len(zippedCoordinates_prostate) == 0
                and not len(zippedCoordinates_indices_tumor) == 0
            ):
                number_of_voxels_whole_prostate_segmentation = np.where(
                    self.prostate_gland_arr_slice
                )
                sample_size = self.sample_size_calculation(
                    number_of_voxels_whole_prostate_segmentation[0].size
                )
                        ...
                        if self.arg.sequence_type == "T2W":
                            if imgaNew.shape == (
                                ...
                                pathToSave = (
                                    ...
                                    / f"{self.slice_name}_{i}_cord_{y1}_{x1}"
                                )
                                saveFilesC.saveFiles(self, str(pathToSave), imgaNew)
                            ...
                            imga_adc = _imageNArray_adc[
                                y1 : y1 + self.arg.crop_image_size,
                                x1 : x1 + self.arg.crop_image_size,
                            ]
                            ...
                            saveFilesC.save_image_types(
                                self, self.slice_name, x1, y1, imgaNew, imga_adc, imga_hbv, count
                            )
