...
MODALITIES_CONFIG = PROJECT_ROOT / "config" / "modalities.ini"
...
class saveFilesC:
    ...
    def saveImage(
        self,
        pathToSave: pathlib.Path,
        image_array: np.ndarray,
        vmin: float | None,
        vmax: float | None,
        saved_image_type: str,
    ) -> None:
        if saved_image_type == "npy":
            try:
                np.save(pathToSave.with_suffix(".npy"), image_array)
            except ValueError as exp:
                print("Error", exp)
        else:
            try:
                pathToSave.parent.mkdir(parents=True, exist_ok=True)
    ...
    def save_image_types(
        self, slice_name, final_x1, final_y1, t2w_sub_img, ADC_sub_img, HBV_sub_img, count=None
    ):
        ...
        def do_normalization_method(
            image_array: np.ndarray, min_percentile: float, max_percentile: float, path_to_file: str
        ) -> tuple[float, float]:
            """
        ...
        def do_gaussian_normalization(self, image_array: np.ndarray) -> tuple[np.ndarray, float, float]:
            """
            Normalize image using Gaussian (z-score) normalization.
            ...
            -------
            normalized_array : np.ndarray
                The normalized image array (z-scores clipped to [-3, 3] and rescaled to [0, 1]).
            vmin : float
                Minimum value for display (0).
            vmax : float
                Maximum value for display (1).
            ...
            # Clip to [-3, 3] (covers ~99.7% of normal distribution) and rescale to [0, 1]
            clipped = np.clip(z_scores, -3, 3)
            normalized_array = (clipped + 3) / 6  # Maps [-3, 3] to [0, 1]
        
            return normalized_array.astype(np.float32), 0.0, 1.0
        ...
        def saveFiles(pathToSave: pathlib.Path, image_array: np.array):
            ...
            if self.T2W in str(pathToSave):
                if self.normalized_image:
                    saveImage(
                        pathToSave=finalpathToSave,
                        image_array=image_array,
                        vmin=0,
                        vmax=self.normalized_vmaxNumber,
                        saved_image_type=self.saved_image_type,
                    )
                else:
                    if self.do_normalization:
                        minVal, maxVal = do_normalization_method(
                            image_array,
                            self.min_percentile,
                            self.max_percentile,
                            self.orig_img_path_t2w,
                ...
                print(
                    "The path needs to contain the word ADC, T2W or HBV: See config/modalities.ini"
                )
